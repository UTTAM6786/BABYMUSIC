        ...
        try:
            ...
            if afktype == "text":
                send = await message.reply_text(
                    f"**{message.from_user.first_name}** ιʂ Ⴆαƈƙ σɳʅιɳҽ αɳԃ ɯαʂ αɯαყ ϝσɾ {seenago}",
            ...
            if afktype == "animation":
                if str(reasonafk) == "None":
                    send = await message.reply_animation(
                        data,
                        caption=f"**{message.from_user.first_name}** ιʂ Ⴆαƈƙ σɳʅιɳҽ αɳԃ ɯαʂ αɯαყ ϝσɾ {seenago}",
                    )
                else:
                    send = await message.reply_animation(
                        data,
            ...
            send = await message.reply_text(
                f"**{message.from_user.first_name}** ιʂ Ⴆαƈƙ σɳʅιɳҽ",
                disable_web_page_preview=True,
            )
...
async def chat_watcher_func(_, message):
    ...
    if verifier:
        ...
        try:
            ...
            seenago = get_readable_time((int(time.time() - timeafk)))
            if afktype == "text":
                msg += f"**{user_name[:25]}** ιʂ Ⴆαƈƙ σɳʅιɳҽ αɳԃ ɯαʂ αɯαყ ϝσɾ {seenago}\n\n"
            if afktype == "text_reason":
                msg += f"**{user_name[:25]}** ιʂ Ⴆαƈƙ σɳʅιɳҽ αɳԃ ɯαʂ αɯαყ ϝσɾ {seenago}\n\nʀᴇᴀsᴏɴ: `{reasonafk}`\n\n"
            if afktype == "animation":
                if str(reasonafk) == "None":
                    send = await message.reply_animation(
                        data,
                        caption=f"**{user_name[:25]}** ιʂ Ⴆαƈƙ σɳʅιɳҽ αɳԃ ɯαʂ αɯαყ ϝσɾ {seenago}\n\n",
                    )
                else:
                    send = await message.reply_animation(
                        data,
        ...
        try:
            ...
            if verifier:
                try:
                    ...
                    if afktype == "text_reason":
                        msg += f"**{replied_first_name[:25]}** ιʂ αϝƙ ʂιɳƈҽ {seenago}\n\nʀᴇᴀsᴏɴ: `{reasonafk}`\n\n"
                    if afktype == "animation":
                        if str(reasonafk) == "None":
                            send = await message.reply_animation(
                                data,
                                caption=f"**{replied_first_name[:25]}** ιʂ αϝƙ ʂιɳƈҽ {seenago}\n\n",
                            )
                        else:
                            send = await message.reply_animation(
                                data,
            ...
            if (entity[j].type) == MessageEntityType.MENTION:
                ...
                if verifier:
                    try:
                        ...
                        seenago = get_readable_time((int(time.time() - timeafk)))
                        if afktype == "text":
                            msg += f"**{first_name[:25]}** ιʂ αϝƙ ʂιɳƈҽ {seenago}\n\n"
                        if afktype == "text_reason":
                            msg += f"**{first_name[:25]}** ιʂ αϝƙ ʂιɳƈҽ {seenago}\n\nʀᴇᴀsᴏɴ: `{reasonafk}`\n\n"
                        if afktype == "animation":
                            if str(reasonafk) == "None":
                                send = await message.reply_animation(
                                    data,
                                    caption=f"**{first_name[:25]}** ιʂ αϝƙ ʂιɳƈҽ {seenago}\n\n",
                                )
                            else:
                                send = await message.reply_animation(
                                    data,
