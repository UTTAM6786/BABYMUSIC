...
async def start_pm(client, message: Message, _):
    ...
    if len(message.text.split()) > 1:
        ...
        if name[0:3] == "sud":
            ...
            if await is_on_off(2):
                return await app.send_message(
                    ...
                    text=f"{message.from_user.mention} ʝυʂƚ ʂƚαɾƚҽԃ ƚԋҽ Ⴆσƚ ƚσ ƈԋҽαƙ <b>sᴜᴅᴏʟɪsᴛ</b>.\n\n<b>ᴜsᴇʀ ɪᴅ :</b> <code>{message.from_user.id}</code>\n<b>ᴜsᴇʀɴᴀᴍᴇ :</b> @{message.from_user.username}",
                )
            return
        if name[0:3] == "inf":
            ...
            if await is_on_off(2):
                return await app.send_message(
                    chat_id=config.LOGGER_ID,
                    text=f"{message.from_user.mention} ʝυʂƚ ʂƚαɾƚҽԃ ƚԋҽ Ⴆσƚ ƚσ ƈԋҽαƙ <b>ᴛʀᴀᴄᴋ ɪɴғᴏʀᴍᴀᴛɪᴏɴ</b>.\n\n<b>ᴜsᴇʀ ɪᴅ :</b> <code>{message.from_user.id}</code>\n<b>ᴜsᴇʀɴᴀᴍᴇ :</b> @{message.from_user.username}",
                )
    else:
        out = private_panel(_)
        ...
        if await is_on_off(2):
            return await app.send_message(
                chat_id=config.LOGGER_ID,
                text=f"{message.from_user.mention} ʝυʂƚ ʂƚαɾƚҽԃ ƚԋҽ Ⴆσƚ.\n\n<b>ᴜsᴇʀ ɪᴅ :</b> <code>{message.from_user.id}</code>\n<b>ᴜsᴇʀɴᴀᴍᴇ :</b> @{message.from_user.username}",
            )
